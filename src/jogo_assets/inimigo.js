...
function zumbisSpawn() {
    ...
    while (spawnados < vezes) {
        ...
        zumbi.style.top = `${mapa_altura * 0.50 + local_spawn * 200}px`
        zumbi.style.left = `${((local_spawn * 150) * spawnados)}px`
...
setInterval(() => {
    if(comecou){counter.textContent = `Zumbis: ${zumbisArray.length}`}
    audio_zumbi_passos.pause()
    audio_zumbi_passos.play()
...
//  Efeito sonoro dos zumbis

let timer = 3000

randomFalas()
function randomFalas() {
    let fala = Math.floor(Math.random() * 5)

    audio_zumbi_fala = new Audio(`jogo_assets/assets/audios/zumbi_fala_${fala + 1}_sound.aac`)
    audio_zumbi_fala.play()
